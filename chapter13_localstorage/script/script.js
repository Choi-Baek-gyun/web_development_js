 ...
 * 2. 할 일 목록을 랜더링 : 로컬 스토리지에 저장된 데이터가 있는 경우 불러오기
...
function addTodo() {
  ...
  if (newTodo) {
    // newTodo는 기본적으로 str -> 값이 있기만 하면 true
    todos.push(newTodo);
...
function renderTodos(todos) {
  ...
  todos.array.forEach((todo, index) => {
    ...
    // 삭제 버튼은 renderTodos() 호출했을 때 <li>가 만들어지고, <button>이 만들어지기 때문에
    // 기능과 관련된 부분이 없습니다. -> renderTodos() 내부에 button의 삭제 기능을 구현해둬야 할 겁니다.

    // 물론 우리가 배운대로 외부에 구현해놓고 여기서 호출만 해도 되는데 매커니즘을 설명하기 위해 여기에 작성하겠습니다.

    deleteBtn.onclick = () => removeTodo(index); // 버튼을 클릭하면 removeTodo() 함수를 호출하겠다
...
function removeTodo(index) {
  ...
  todos.splice(index, 1); // 버튼이 눌려진 index넘버의 element만 삭제 -> 배열에서만 삭제
  // 배열에서만의 삭제가 의미하는 바? -> 로컬스토리지에는 남아있다 -> 그렇다면 다시 읽어오게 된다면
